...
then   #Si existe path al archivo inicializador toma las coordenadas del mismo y las concatena al POSCAR
   python programs/initializer.py input.bh
   tail -$Nat $path >>  $Simbolo_1$N_Simbolo_1$Simbolo_2$N_Simbolo_2/POSCAR
else  #De otra forma  invoca al generador aleatorio
   python programs/starting.py input.bh
fi
...
rm -r $Simbolo_1$N_Simbolo_1$Simbolo_2$N_Simbolo_2
python programs/starting.py input.bh
cd $Simbolo_1$N_Simbolo_1$Simbolo_2$N_Simbolo_2
mkdir rejected
./run.sh
